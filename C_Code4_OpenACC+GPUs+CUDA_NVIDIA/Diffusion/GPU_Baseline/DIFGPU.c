                ...
                REAL cb, REAL cc, int time)
{
  ...
  #pragma acc data copy(F1[0:(nx)*(ny)*(nz)],F2[0:(nx)*(ny)*(nz)])
 {
  for (t = 0; t < time; ++t)
  {
    #pragma acc parallel loop num_gangs(4) num_workers(16) vector vector_length(64) present(F1,F2)
    for (z=0; z < nz; z++)
...
{
#pragma acc parallel loop num_gangs(4) num_workers(16) vector vector_length(64)
  for (jz = 0; jz < nz; jz++)
#pragma acc loop worker
    for (jy = 0; jy < ny; jy++)
#pragma acc loop gang
