...
int main(int argc, char *argv[])
{
  ...
  // Calculate time taken by init()
  clock_t tinit;
  tinit = clock();
  init(f1, NX, NY, NZ, kx, ky, kz, dx, dy, dz, kappa, time);
  tinit = clock() - tinit;
  double time_taken_init = ((double)tinit)/CLOCKS_PER_SEC;
  printf("init() took %f seconds to execute \n", time_taken_init);

  // Calculate time taken by sum_values()
  clock_t tsum;
  tsum = clock();
  REAL err = sum_values(f1, NX, NY, NZ);
  tsum = clock() - tsum;
  double time_taken_sum = ((double)tsum)/CLOCKS_PER_SEC;
  printf("sum_values() took %f seconds to execute \n", time_taken_sum);
  ...
  // Calculate time taken by diffusion()
  clock_t tdif;
  tdif = clock();
  diffusion(f1, f2, NX, NY, NZ, ce, cw, cn, cs, ct, cb, cc, count);
  tdif = clock() - tdif;
  double time_taken_dif = ((double)tdif)/CLOCKS_PER_SEC;
  printf("diffusion() took %f seconds to execute \n", time_taken_dif);
