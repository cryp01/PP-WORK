                ...
                REAL cb, REAL cc, int time)
{
  ...
  #pragma acc data copy(F1[0:(nx)*(ny)*(nz)],F2[0:(nx)*(ny)*(nz)])
 {
  for (t = 0; t < time; ++t)
  {
    #pragma acc parallel loop num_gangs(4) num_workers(8) vector vector_length(128) present(F1,F2)
    for (z=0; z < nz; z++)
           ...
           const REAL kappa, const REAL time)
{
 ...
 #pragma acc data copy(F1[0:(nx)*(ny)*(nz)])
{
  #pragma acc parallel loop num_gangs(4) num_workers(8) vector vector_length(128)
  for (jz = 0; jz < nz; jz++)
 #pragma acc loop worker
    for (jy = 0; jy < ny; jy++)
 #pragma acc loop gang
